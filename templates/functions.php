...
if ( ! function_exists( '<%= appNameSlug %>_setup' ) ) :
/**
...
function <%= appNameSlug %>_setup() {
	 ...
	 * to change '<%= appNameSlug %>' to the name of your theme in all the template files
	 */
	...
	add_theme_support( 'custom-background', apply_filters( '<%= appNameSlug %>_custom_background_args', array(
 ...
 */
function <%= appNameSlug %>_content_width() {
	$GLOBALS['content_width'] = apply_filters( '<%= appNameSlug %>_content_width', 640 );
}
add_action( 'after_setup_theme', '<%= appNameSlug %>_content_width', 0 );
 ...
 */
function <%= appNameSlug %>_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', '<%= appNameSlug %>' ),
		'id'            => 'sidebar-1',
		'description'   => '',
...
add_action( 'widgets_init', '<%= appNameSlug %>_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function <%= appNameSlug %>_scripts() {
...
add_action( 'wp_enqueue_scripts', '<%= appNameSlug %>_scripts' );
